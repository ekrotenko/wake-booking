...
// element from the seed arrays above, and log how many are created
  ...
  .then(() => Promise.map(userData, user => db.User.create(user)))
  .then((createdUsers) => {
...
// })
  .then(() => Promise.map(parkData, park => db.Park.create(park)))
  .then((createdParks) => {
    console.log(`${createdParks.length} parks created`);
  })

  .then(() => Promise.map(ropewayData, ropeway => db.Ropeway.create(ropeway)))
  .then((createdRopeways) => {
    console.log(`${createdRopeways.length} ropeways created`);
  })

  .then(() => Promise.map(scheduleData, schedule => db.Schedule.create(schedule)))
  .then((createdSchedules) => {
    console.log(`${createdSchedules.length} schedules created`);
  })
  .then(() => Promise.map(inaccessibleSlotsData, inaccessibleSlot => db.InaccessibleTimeSlot.create(inaccessibleSlot)))
  .then((createdUns) => {
